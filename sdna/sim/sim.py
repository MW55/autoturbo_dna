...
class Sim(object):
    ...
    def _prepare_error_simulation(self, mult):
        """
        ...
        if self.args["synthesis"][0] != "0":
            ...
            syn = ErrorSource(process="synthesis",
                              config_file=syn_json).get_by_id(self.args["synthesis"][0], multiplier=mult)
        ...
        if self.args["storage"][0] != "0":
            ...
            sto = ErrorSource(process="storage",
                              config_file=sto_json).get_by_id(self.args["storage"][0], multiplier=self.args["storage_months"] * mult)
        ...
        if self.args["pcr"][0] != "0":
            ...
            pcr = ErrorSource(process="storage",
                              config_file=pcr_json).get_by_id(self.args["pcr"][0], multiplier=self.args["pcr_cycles"] * mult)
        ...
        if self.args["sequencing"][0] != "0":
            ...
            seq = ErrorSource(process="sequencing",
                              config_file=seq_json).get_by_id(self.args["sequencing"][0], multiplier=mult)
